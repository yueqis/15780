    ...
    for (int oz = 0; oz < out_depth; oz++) {
        for (int oy = 0; oy < out_height; oy++) {
            for (int ox = 0; ox < out_width; ox++) {
                    ...
                    for (int kz = 0; kz < kernel_size; kz++) {
                        for (int ky = 0; ky < kernel_size; ky++) {
                            for (int kx = 0; kx < kernel_size; kx++) {
                                ...
                                if ((oz - kz) % stride == 0 && (oy - ky) % stride == 0 && (ox - kx) % stride == 0 &&
                                    iz >= 0 && iy >= 0 && ix >= 0 &&
                                    iz < depth && iy < height && ix < width) {
                                    
                                    int input_idx = b * in_channels * depth * height * width + ic * depth * height * width + iz * height * width + iy * width + ix;
                                    int weight_idx = oc * in_channels * kernel_size * kernel_size * kernel_size + ic * kernel_size * kernel_size * kernel_size + kz * kernel_size * kernel_size + ky * kernel_size + kx;
                ...
                int output_idx = b * out_channels * out_depth * out_height * out_width + oc * out_depth * out_height * out_width + oz * out_height * out_width + oy * out_width + ox;
                output[output_idx] = (bias != NULL) ? val + bias[oc] : val;
            }
        }
    }
}
...
torch::Tensor conv_transpose3d_cuda(torch::Tensor input, torch::Tensor weight, torch::Tensor bias,
    ...
    conv_transpose3d_kernel<<<blocks, 1>>>(
...
class ModelNew(nn.Module):
    def __init__(
        ...
        # Assign custom op
        self.custom_conv_transpose3d = conv_transpose3d_op.conv_transpose3d_cuda
